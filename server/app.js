...
var adminauth=require('./middlewares/testauth');
var verifiedMessage=require('./models/verifiedMessage');
...
const uploadStorage = multer({ storage: storagepdf })
...
app.post("/users/getverify",auth, uploadStorage.array("file", 10), async(req, res,next) => {
  //console.log(req.files)
  try{
  var result = req.files.map(function(a) {return "pdfimages/"+a.filename;});
console.log(result)
//console.log(req.senderid)
console.log(req.body.sendername)
const newmessage=await verifiedMessage.create({senderid:req.user.id,sendername:req.body.sendername,
senderemail:req.body.senderemail,
messbody:req.body.messbody,otherfiles:result
...
User.findOneAndUpdate({_id:req.user.id},{profileimg:`uploads/${req.file.filename}`},(err,results)=>{
if(err){
  next(err)
}
...
app.post('/admin/:id/updateprofileimage', (req, res,next)=> {
  upload(req, res, (err) => {
    if(err){
      res.json( {
        msg: err
...
Admin.findOneAndUpdate({_id:req.params.id},{profileimg:`uploads/${req.file.filename}`},(err,results)=>{
if(err){
  next(err)
}
...
app.post('/getverified', (req,res,next)=>{
